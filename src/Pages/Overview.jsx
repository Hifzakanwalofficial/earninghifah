// src/Pages/Overview.jsx (FULLY FIXED - EXACT DATES SENT TO BACKEND - NO +1 DAY BUG)
...
// === UTILITY: PARSE YYYY-MM-DD STRING TO LOCAL DATE OBJECT ===
...
const ShimmerChart = () => (
  <div className="bg-white dark:bg-[#080F25] rounded-xl shadow animate-pulse h-[300px] w-full"></div>
);
...
const Overview = () => {
  ...
  // Separate visible month/year for accurate calendar view
  ...
  const fetchStats = async () => {
    ...
    try {
      const url = new URL(`${Baseurl}/driver/stats`);
      ...
      setStats({
        ...
        totalRems: data.totals?.totalRems || 0,
        totalRpm: data.totals?.totalRpm || 0,
        totalPri: data.totals?.totalPr1 || 0,
        grandTotal: data.totals?.grandTotal || 0,
        percentageEarnings: data.totals?.percentageEarnings || 0,
      });
    } catch (err) {
  ...
  // Accurate date selection using local time
  ...
  // Generate calendar days (local dates)
  ...
  const currentMonthName = new Date(visibleYear, visibleMonth).toLocaleDateString("en-US", { month: "long" });
  ...
  if (loading) {
    return (
      <div className="min-h-screen p-6">
        ...
        <div className="mt-6"><Cards fromDate={fromDate} toDate={toDate} loading /></div>
        <div className='mt-[44px] space-y-2'>
          <div className="h-6 w-40 bg-gray-300 dark:bg-gray-700 rounded animate-pulse"></div>
          <div className="h-4 w-72 bg-gray-300 dark:bg-gray-700 rounded animate-pulse"></div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-[#080F25] p-4 sm:p-6">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-[20px] sm:mb-[44px] gap-4">
        <div className="flex justify-between flex-1 w-full sm:w-auto">
          <p className="robotosemibold text-[24px] dark:text-white">Dashboard</p>
          <button 
            onClick={goToForm} 
            className="bg-[#0078BD] text-white w-[124px] rounded-[7px] flex justify-center items-center gap-3 cursor-pointer"
          >
            <FaPlus /> Forms
          </button>
        </div>

<div ref={datePickerRef} className="relative w-fit ml-auto sm:ml-0">
  <div
    onClick={() => setShowDatePicker(!showDatePicker)}
    className="flex items-center justify-between border border-gray-300 rounded px-4 py-2.5 cursor-pointer hover:border-blue-500 bg-white dark:bg-[#101935] dark:text-white w-full sm:w-[250px] dark:border-gray-700"
  >
    <div className="flex items-center space-x-2">
      <svg className="w-4 h-4 text-gray-400 dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
      </svg>
      <span className={`truncate text-[14px] ${!fromDate && !toDate ? "text-gray-400 dark:text-gray-500" : "text-gray-700 dark:text-white"}`}>
        {getDateRangeText()}
      </span>
    </div>
    {(fromDate || toDate) && (
    ...
    <div className="absolute top-full mt-2 right-0 bg-white dark:bg-[#101935] border border-gray-300 rounded-lg shadow-lg z-50 p-3 w-72 dark:border-gray-700">
      <div className="mb-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <div className="flex items-center space-x-1">
            ...
            <select
              value={visibleYear}
              onChange={(e) => setVisibleYear(parseInt(e.target.value))}
      ...
      <div className="grid grid-cols-7 gap-0.5 mb-2">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
          <div key={day} className="text-center text-xs font-medium text-gray-500 py-1 dark:text-gray-400">{day}</div>
        ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-6">
        ...
        <div className="bg-white px-[14px] py-[24px] rounded-xl shadow hover:shadow-lg transition flex items-center justify-between dark:bg-[#101935] dark:border-1 dark:border-[#263463]">
          <div>
            <h2 className="text-[12px] sm:text-[16px] robotomedium text-[#333333B2] dark:text-[#95A0C6]">Total RPM</h2>
            <p className="text-[18px] sm:text-2xl font-bold mt-2 dark:text-white">{stats.totalRpm.toLocaleString()}</p>
          </div>
          <div className='bg-[#778da93f] h-[40px] w-[40px] rounded-full flex items-center justify-center'>
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="bg-white px-[14px] py-[24px] rounded-xl shadow hover:shadow-lg transition flex items-center justify-between dark:bg-[#101935] dark:border-1 dark:border-[#263463]">
          <div>
            <h2 className="text-[12px] sm:text-[16px] robotomedium text-[#333333B2] dark:text-[#95A0C6]">Total PR1</h2>
            <p className="text-[18px] sm:text-2xl font-bold mt-2 dark:text-white">{stats.totalPri.toLocaleString()}</p>
          </div>
          <div className='bg-[#778da93f] h-[40px] w-[40px] rounded-full flex items-center justify-center'>
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <g clip-path="url(#clip0_991_10419)">
        ...
        <div className="bg-white px-[14px] py-[24px] rounded-xl shadow hover:shadow-lg transition flex items-center justify-between col-span-2 lg:col-span-1 dark:bg-[#101935] dark:border-1 dark:border-[#263463]">
          <div>
            <h2 className="text-[12px] sm:text-[16px] robotomedium text-[#333333B2] dark:text-[#95A0C6]">Total Earning</h2>
            {/* <p className="text-[18px] sm:text-2xl font-bold mt-2 dark:text-white">${stats.grandTotal.toLocaleString()}</p> */}
                        <p className="text-[18px] sm:text-2xl font-bold mt-2 dark:text-white">${stats.percentageEarnings.toLocaleString()}</p>

          </div>
          <div className='bg-[#778da93f] h-[40px] w-[40px] rounded-full flex items-center justify-center'>
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M17.5 17.5H3.83333C3.36662 17.5 3.13327 17.5 2.95501 17.4092C2.79821 17.3293 2.67072 17.2018 2.59083 17.045C2.5 16.8667 2.5 16.6334 2.5 16.1667V2.5M17.5 5.83333L12.9714 10.3619C12.8064 10.5269 12.7239 10.6094 12.6288 10.6404C12.5451 10.6675 12.4549 10.6675 12.3712 10.6404C12.2761 10.6094 12.1936 10.5269 12.0286 10.3619L10.4714 8.80474C10.3064 8.63973 10.2239 8.55723 10.1288 8.52632C10.0451 8.49912 9.95493 8.49912 9.87124 8.52632C9.77611 8.55723 9.6936 8.63973 9.5286 8.80474L5.83333 12.5M17.5 5.83333H14.1667M17.5 5.83333V9.16667" stroke="#0078BD" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        </div>
      </div>
      ...
      <div className='mt-6'>
        <Cards fromDate={fromDate} toDate={toDate} />
      </div>

      <div className='mt-[44px]'>
        <p className="robotomedium text-[16px] sm:text-[20px] dark:text-white">Monthly Performances</p>
        <p className="robotoregular text-[12px] sm:text-[16px] text-[#707070] dark:text-gray-400">Overview of calls, Pr1, Rems, and Rpm over the selected period.</p>
      </div>

      <div className="flex flex-col lg:flex-row gap-3 mt-6">
        <Linechart fromDate={fromDate} toDate={toDate} />
        <CallByClientsChart fromDate={fromDate} toDate={toDate} />
      </div>
    </div>
  );
};
